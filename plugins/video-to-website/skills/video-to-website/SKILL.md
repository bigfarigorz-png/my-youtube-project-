...
**Prerequisite: FFmpeg and FFprobe.** Verify with `ffmpeg -version` and `ffprobe -version`. If they are missing, install before extracting frames — e.g. `apt-get update && apt-get install -y ffmpeg` (Debian/Ubuntu), `brew install ffmpeg` (macOS), or a static build on Windows. Do NOT reinstall if they are already on PATH.
